import {IconBox, ImageVeiw} from "@/components";
...
export function Banner({title, subTitle, image, bg_image}: Props) {
    //TODO form logic
    return (
        <>
            <div style={{backgroundImage: `url('${bg_image}')`}} className="grid grid-cols-2 items-center rounded-lg bg-[#FDC040] bg-opacity-20 bg-cover bg-no-repeat">
                <div className="pl-[20px] md:pl-[60px]">
                    ...
                    <form method="post" action="#" className="hidden lg:flex rounded-[30px] pl-[26px] lg:w-[442px] w-full bg-white items-center">
                        <IconBox className={"icon-paper-plane"} size= {24}/>
                ...
                <ImageVeiw src={`${image}`} className="m-w-[50%]" width={717} height={538} alt={"banner"}/>
            </div>
            <form name="subscription" method="post" action="#" className="flex lg:hidden gap-2.5 items-center justify-between rounded-[30px] mx-auto max-w-[550px] bg-[#F5F5F5] pl-4 sm:pl-5 md:pl-6 mt-4 sm:mt-5 md:mt-6">
                <IconBox className={"icon-paper-plane"} size={24} link={"#"}/>
