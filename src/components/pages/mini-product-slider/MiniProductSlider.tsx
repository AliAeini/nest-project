        ...
        >
            {
                miniProductSlider.map((item, index)=>{
                    return(
                        <SwiperSlide>
                            <div style={{backgroundImage: `url(${item.image})`, backgroundColor: `${item.color}`}} key={index} className="flex items-center aspect-slider pl-4 lg:pl-12 rounded-[10px] bg-[length:50%] bg-no-repeat bg-right-bottom">
                                <div className="basis-1/2 flex flex-col justify-between">
                                    ...
                                    <Link href={item.link} className="mt-6 pl-[15px] pr-2.5 py-2 bg-green-200 hover:bg-yellow-100 rounded-[3px] cursor-pointer inline-flex max-w-max items-center gap-2.5">
                                        <div className="text-small text-white">Shop now</div>
